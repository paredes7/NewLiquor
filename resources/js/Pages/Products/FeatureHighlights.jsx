export default function FeatureHighlights({ specifications }) {
  const features = [
    ...
    },
    {
      ...
      value: specifications.capacidad_carga,
    },
  ];
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {features.map((feature, idx) => (
        <div
          key={idx}
          className="bg-gray-50 p-4 rounded-xl text-center space-y-3 hover:shadow-lg transition-shadow"
        >
          <div className="w-12 h-12 mx-auto text-darkGray">{feature.icon}</div>
          <div>
            <p className="text-xs font-bold text-grayCustom uppercase tracking-wide">
              {feature.label}
            </p>
            <p className="text-sm font-bold text-darkGray mt-1">
              {feature.value}
            </p>
          </div>
        </div>
